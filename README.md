...
| Collectibles | regular NFTs yes, read from the chain; **compressed NFTs no** on Mainnet — those need DAS | yes, both kinds |
...
- **Keyless Mainnet cannot see compressed NFTs.** A compressed NFT has no mint account and no token account — it is a leaf in a Merkle tree, and reconstructing ownership means replaying that tree's transaction log, which is what a DAS indexer is for. No free endpoint serves DAS, and Metaplex retired the free Aura endpoints (the hostnames are NXDOMAIN on public resolvers). This is a property of the data structure, not a gap to engineer around, and the gallery says so rather than letting a partial grid read as a whole collection. Regular NFTs *are* readable keylessly and are shown; see ADR 0005.
- **A keyless collectible's ownership is discovered, not proved.** The mints come from Jupiter, as the token list's do; the mint and metadata accounts the wallet reads afterwards name no owner. Nothing on this tab is spendable, sendable or burnable, so a stale entry costs a picture rather than a transaction — and confirming each item's token account would double the tab's chain reads for a display-only surface.
...
- **The one-of-one filter is on the keyless path only.** Splitting collectibles out of the token list reads each mint's supply, which the keyless path had already fetched. `getTokenAccountsByOwner` does not return supply, so with your own RPC URL or a Helius key a one-of-one still shows as a token row of "1" — and also, properly, in the collectibles tab, which DAS then fills. Closing that would add a whole extra chain read to the fastest token path for a cosmetic gain; see ADR 0005.
...
- **A keyless refresh confirms at most 200 mints.** Each ten cost a round trip through the single public host, and wallets holding thousands of airdropped mints are real. Whatever is past the cap, or whose mint account could not be read, is counted under the list rather than dropped in silence. Collectibles are discovered out of that same 200, and the count of what was not asked about appears under the token list — the collectibles tab names the cap rather than repeating the number.
