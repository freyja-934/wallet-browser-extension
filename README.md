...
Vault (v2): PBKDF2-SHA256 600k + AES-256-GCM in chrome.storage.local, parameters stored in the blob
...
- Wallet Standard: `standard:connect` (with `silent`), `standard:disconnect`, `standard:events`, `solana:signTransaction`, `solana:signAndSendTransaction`, `solana:signMessage`. The wallet lists `solana:mainnet` and `solana:devnet`; each account's `chains` is the active cluster and is re-stamped on a cluster change, so wallet-adapter can send on either. A request naming the other cluster is refused with "Cinder is on Devnet; switch networks in Settings"; `solana:testnet` and `solana:localnet` are not supported. N inputs to one `signTransaction` or `signMessage` call (at most 10, and at most 256 KiB in total) are one approval window and N outputs in order, active account first; `signAndSendTransaction` takes exactly one transaction per call (a batch broadcast is not atomic), forwards `skipPreflight`, `preflightCommitment`, `maxRetries`, `minContextSlot`, and waits for `commitment` for at most 30 s, stopping 5 s before the page's own timeout. Switching the cluster in Settings rejects a pending transaction request with "Network changed" and re-stamps every connected page's account `chains`. `signMessage` refuses bytes that are a serialized transaction message. The provider is a MAIN-world content script (Chrome 111 or later), not a `<script>` tag.
- Vault v2: the blob carries its own version and KDF parameters (`{ version, kdf: { name, hash, iterations }, salt, nonce, ciphertext }`), PBKDF2-SHA256 at OWASP's 600,000 iterations with AES-256-GCM. A v1 blob (unversioned, 100k) is re-encrypted at the current version the first time its owner unlocks, written before anything else in that unlock; a blob this build cannot read is refused as `Unsupported vault format` rather than reported as a wrong password. Unlock, export, and change-password each derive the key once, and exporting a secret or changing the password never unlocks a locked wallet. See `docs/adr/0002-vault-v2.md`.
- Multiple accounts: the header switcher lists, adds (next `m/44'/501'/n'/0'`), renames, and switches accounts. The list and the selected account are public state in `chrome.storage.local`, so both survive a lock — the unlock comes back on the account you were using — and the account list changes reach connected sites as a Wallet Standard `change` with the active address first. Accounts are looked up by their derivation index, never by position in the list, and the three list writes are serialised in the worker so two at once cannot drop one another.
...
- Simulation preview: address lookup tables resolved, a balance diff (SOL and tokens before → after, from `getMultipleAccountsInfoAndContext` and a `simulateTransaction` with `replaceRecentBlockhash` pinned to that read's slot via `minContextSlot`, capped at 32 accounts and marked partial past that), program names, `setAuthority` / approve warnings, unknown programs. Approve is disabled while the preview is unsettled, when an instruction is unreadable, or when the active account is not a required signer; a failed simulation leaves a danger-styled "Approve anyway".
...
just e2e        # Playwright: import, unlock, dashboard, create, second-create refusal, send (confirms a 0.001 SOL devnet self-transfer), settings, accounts, dApp
...
- Coverage: import / unlock, dashboard tabs, create-new + seed quiz, a second CREATE_WALLET against an existing vault refused rather than replacing it, Send → Review (junk address, Max as balance minus the priced fee, the decimals error, the fee row) and one confirmed 0.001 SOL self-transfer on devnet — the fixture sends to itself, so only the fee leaves it — settings auto-lock / export seed / change password, add / switch / rename a second account (Enter commits a rename, clicking away abandons it) with the account list and the selected account surviving a lock, Receive toast stub, dApp connect (with `solana:devnet` in `chains`) / sign-message / sign-v0, two transactions in one call → one approval window and outputs in input order, cluster switch → `change` with re-stamped `chains`, wrong-chain refusal, transaction-as-message refusal, locked Connect → unlock inside the approval window, `signAndSend` reject, 0-lamport `signAndSend` approve with the fee shown as the SOL change, approval window close → reject, repeat and silent connect without a window, Settings → Connected sites → Revoke, `Not connected` for a stranger, lock → empty `change` event.
