...
/** Every host the devnet popup may read from: the seeded Helius key, the public devnet node, publicnode. */
const RPC_HOSTS = /(helius-rpc\.com|api\.devnet\.solana\.com|solana-rpc\.publicnode\.com)/;

/** Rows per history page in `useTransactions`. */
const HISTORY_PAGE_SIZE = 20;

/** Ask devnet, from the test process, whether the fixture has more than one page of history. */
...
test('activity pages with Load more when the fixture has more than one page', async ({ context, extensionId }) => {
  ...
  await popup.getByTestId('nav-activity').click();
  const rows = popup.getByTestId('activity-amount');
  const loadMore = popup.getByTestId('history-load-more');

  if (signatures > HISTORY_PAGE_SIZE) {
    ...
    await expect(popup.getByText('No activity yet').or(rows.first())).toBeVisible({ timeout: 30_000 });
