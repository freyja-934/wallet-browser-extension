...
You can create or import a wallet, send SOL and tokens, view collectibles, and connect to sites that use Wallet Standard. Approvals open a dedicated window with a simulation preview when the RPC allows it.

On Mainnet with no endpoint of your own, Cinder uses a public RPC for balances, sends and history. That endpoint will not list a wallet's token accounts, so the list of which tokens you hold — and their names and logos — comes from Jupiter's free public API, which receives your public address for that request alone. Every figure is then read from the Solana chain, not from Jupiter: each mint's own account is fetched for its decimals, and your own token account of that mint for the balance shown. A holding the chain does not confirm is left off the list rather than guessed at, and the popup says how many. Enter your own RPC URL or a Helius API key in Settings and Jupiter is not asked at all; a key or URL stays on your device. Regular NFTs are read from the chain the same way: the mints that turn out to hold one indivisible unit are shown as collectibles rather than as a token balance of 1, and each one's name and picture come from its own on-chain metadata account and from whatever host that account points at, fetched only for the items you are looking at. Compressed NFTs — the cheap kind used for airdrops and free mints — are not listed at all, because they have no account of their own for anyone to read; the tab says so. They and enriched history still need an endpoint of your own. If the public endpoint is unreachable on your network, Cinder tells you so and asks for an endpoint instead of showing a balance it does not know.
...
- Remote: Solana JSON-RPC (publicnode by default on Mainnet, the public devnet host on Devnet, or a URL / Helius key the user enters in Settings) receives public addresses and signed transactions; lite-api.jup.ag receives the public wallet address on Mainnet, and only when the user has configured no RPC URL and no Helius key, to say which mints that address holds and what they are called; CoinGecko receives public mints on Mainnet; the metadata and image hosts named by a token's or collectible's own on-chain metadata receive those requests, and for a collectible only when its tab is open and only for the items shown; solana.fm receives a transaction signature when the user clicks through to the explorer
- Certify Limited Use
...
- **https://lite-api.jup.ag/*** — Keyless Mainnet token discovery only. The public Mainnet RPC refuses `getTokenAccountsByOwner`, so without this a user who configures nothing sees SOL and none of the tokens they hold. Two read-only GETs: which mints an address holds, and the names, symbols and logos for those mints. Requested only on Mainnet, and only while the user has entered neither a custom RPC URL nor a Helius API key — either one turns it off. Balances, decimals and token programs are read from the Solana endpoint, never from this host. Disclosed in the privacy policy.
