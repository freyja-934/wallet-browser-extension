...
1. Files: `src/lib/vault-crypto.ts`, `src/lib/vault-crypto.test.ts`, `src/background/keyring.ts`, new `src/background/keyring.test.ts`, new `docs/adr/0002-vault-v2.md`
   Vault v2: `{ version: 2, kdf: { name: 'PBKDF2', hash: 'SHA-256', iterations: 600000 }, salt, nonce, ciphertext }`; `encrypt` writes v2, `decrypt` reads v1 (no `version`, 100k) and v2 by their own parameters, and refuses an unknown version or KDF with `Unsupported vault format` before deriving anything. `keyring.ts`: `decryptVault` returns the payload plus the vault version and runs the format check outside its try, so only a real decrypt or parse failure becomes `Invalid password`; `unlock` re-encrypts a v1 vault as v2 after a successful decrypt, writes the new blob before removing `lumen_vault`, and restores `activeAccountIndex` from the session when one is open and from `cinder_accounts` otherwise (clamped to an account that still exists); a private `unlockWithPayload(payload)` lets `exportSeed`, `exportPrivateKey`, and `changePassword` derive the key exactly once, and those three refresh the session only when one already exists, so a locked wallet stays locked; `createWallet` refuses when a vault already exists and enforces `validatePasswordStrength`, as `changePassword` does. Tests (chrome stub): create → lock → unlock; v1 fixture → unlock → stored blob is v2 and unlocks again; wrong password; tamper detection; an unreadable format refused with the vault untouched; salt and nonce differ between encrypts; create and change-password reject weak passwords; a second create is refused; export on a locked wallet leaves it locked. ADR records the iteration count, the measured cost (about 0.15 s on a laptop, up to 1 s on low-end hardware), the migration, and that the active account now persists.
   Verify: `just test src/background/keyring.test.ts src/lib/vault-crypto.test.ts`; `just check`.

2. Files: `src/background/keyring.ts`, `src/background/router.ts`, `src/lib/protocol.ts`, `src/popup/App.tsx`
   Idle auto-lock: `touchActivity()` re-arms the alarm on every extension-page message (already called from the router since SHIP-5); the alarm fires only when no activity happened for the configured minutes. Accounts: `cinder_accounts` (public) is the source of truth for the account list *and* for which account is selected — the selection names an address, not a secret, so it is stored with the list and survives a lock, while the session copy stays the live value. `ADD_ACCOUNT` derives the next index and appends; `RENAME_ACCOUNT { index, name }`; `SWITCH_ACCOUNT` emits the SHIP-5 change event and records the new selection; all three run through one promise chain in the keyring so two at once cannot drop each other's write; `unlock` derives from the stored account count. Accounts are addressed by their derivation `index`, never by position in the list. `App.tsx` re-runs `initializeWallet()` on the `locked` event (from SHIP-5) and shows the unlock screen.
